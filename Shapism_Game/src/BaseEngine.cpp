...
static vector<DisplayableObject*>* displayableObjects;
...
void BaseEngine::CreateObjectArray(int iNumberObjects)
{
	...
	displayableObjects = new vector<DisplayableObject*>(iNumberObjects + 1, 0);
...
{
	(*displayableObjects)[iIndex] = pObject;
}
...
void BaseEngine::DestroyOldObjects()
{
	...
	{
		for (auto object : *displayableObjects)
		{
			delete object;
			object = NULL;
		}
		delete displayableObjects;
...
void BaseEngine::ResizeObjectArray(int newSize)
{
	displayableObjects->resize(newSize);
}
...
void BaseEngine::UpdateAllObjects( int iCurrentTime )
{
	...
	if (displayableObjects != NULL)
	{
		...
		{
			(*displayableObjects)[i]->DoUpdate(iCurrentTime);
...
void BaseEngine::UndrawObjects()
{
	...
	if (displayableObjects != NULL)
	{
		for (int i = 0; (*displayableObjects)[i] != NULL; i++)
		{
			(*displayableObjects)[i]->RedrawBackground();
			if (m_iDrawableObjectsChanged)
				return;
		}
	}
...
int BaseEngine::NotifyAllObjectsGetSum( int iSignalNumber )
{
	...
	if (displayableObjects != NULL)
	{
		for (int i = 0; (*displayableObjects)[i] != NULL; i++)
		{
			iReturn += (*displayableObjects)[i]->Notify(iSignalNumber);
		}
	}
...
int BaseEngine::NotifyAllObjectsGetMax( int iSignalNumber )
{
	...
	if (displayableObjects != NULL)
	{
		for (int i = 0; (*displayableObjects)[i] != NULL; i++)
		{
			int ival = (*displayableObjects)[i]->Notify(iSignalNumber);
			if (ival > iReturn)
				iReturn = ival;
		}
	}
